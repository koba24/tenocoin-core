...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw runtime_error(
            ...
            "5. \"anon-destination\"  (string, optional) Encrypted destination address if you're sending a TENOtech transaction \n"
...
UniValue createproposal(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 4)
        throw runtime_error(
            ...
            "\nCreates a proposal for the community fund. Min fee of " + std::to_string((float)Params().GetConsensus().nProposalMinimalFee/COIN) + "TENO is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
...
UniValue createpaymentrequest(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nCreates a proposal to withdraw funds from the community fund. Fee: 0.0001 TENO\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    std::string Secret = "I kindly ask to withdraw " + std::to_string(nReqAmount) + "TENO from the proposal " + proposal.hash.ToString() + ". Payment request id: " + id;
...
UniValue anonsend(const UniValue& params, bool fHelp)
{
    ...
    int nEntropy = GetArg("anon_entropy",TENOTECH_DEFAULT_ENTROPY);
    ...
    UniValue tenotechData = tenotech.CreateAnonTransaction(params[0].get_str(), nAmount / (nTransactions * 2), nTransactions);
    std::vector<UniValue> serverTenoAddresses(find_value(tenotechData, "anonaddress").getValues());

    if(serverTenoAddresses.size() != nTransactions)
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "TENOTech server returned a different number of addresses.");

    for(unsigned int i = 0; i < serverTenoAddresses.size(); i++)
    {
        CTenoCoinAddress serverTenoAddress(serverTenoAddresses[i].get_str());
        if (!serverTenoAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Tenocoin address provided by TENOTech server");
    }
    ...
    CAmount nMinAmount = find_value(tenotechData, "min_amount").get_int() * COIN;
    UniValue pubKey = find_value(tenotechData, "public_key");
    ...
    for(unsigned int i = 0; i < serverTenoAddresses.size(); i++)
    {
        ...
        if(i == serverTenoAddresses.size() - 1 || (nAmountNotProcessed - nAmountToSubstract) < (nMinAmount + 0.001))
        {
            nAmountRound = nAmountNotProcessed;
            i = serverTenoAddresses.size();
        }
        else
        {
        ...
        string encryptedAddress = tenotech.EncryptAddress(params[0].get_str(), pubKey.get_str(), serverTenoAddresses.size(), i+(i==serverTenoAddresses.size()?0:1), nId);
        wtx.strDZeel = encryptedAddress;
        SendMoney(serverTenoAddress.Get(), nAmountRound, fSubtractFeeFromAmount, wtx, encryptedAddress);
    }
...
UniValue getanondestination(const UniValue& params, bool fHelp)
{
    ...
    UniValue tenotechData = tenotech.CreateAnonTransaction(params[0].get_str());

    CTenoCoinAddress serverTenoAddress(find_value(tenotechData, "anonaddress").get_str());
    if (!serverTenoAddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Tenocoin address provided by TENOTech server");
...
UniValue resolveopenalias(const UniValue& params, bool fHelp)
{
  ...
  if ((fHelp || params.size() != 1))
      throw runtime_error(
          ...
          + HelpExampleCli("resolveopenalias", "\"donate@tenocoin.org\"")
      );
