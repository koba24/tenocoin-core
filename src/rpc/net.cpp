...
UniValue addanonserver(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3 ||
        ...
        throw runtime_error(
            ...
            "\nAttempts add or remove a node from the TENOTech server list.\n"
            "\nArguments:\n"
            "1. \"node\"     (string, required) The node (see getpeerinfo for nodes)\n"
    ...
    if (strCommand == "add")
    {
      if (params.size() == 3 && params[2].get_str() == "true") {
        ...
        if (it != vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_ALREADY_ADDED, "Error: TENOTech server already added");
        vAddedAnonServers.push_back(strNode);
      }
    }
    else if(strCommand == "remove")
    {
      if (params.size() == 3 && params[2].get_str() == "true") {
        ...
        if (it == vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_NOT_ADDED, "Error: TENOTech server has not been added.");
        vAddedAnonServers.erase(it);
      }
    }
    return NullUniValue;
...
UniValue getstakesubsidy(const UniValue& params, bool fHelp)
{
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TENO decode failed");
    }
